...
export const AdvertisingStepForm: React.FC<AdvertisingStepFormProps> = ({ children, asDialog = false }) => {
      ...
      try {
        ...
        const { data: quoteEmailResp, error: quoteEmailInvokeErr } = await supabase.functions.invoke('send-booking-confirmation-email', {
          body: {
            ...
            bogof_free_areas: (campaignData.bogofFreeAreas || []).map(id => areas?.find(a => a.id === id)?.name || id),
            total_circulation: campaignData.pricingBreakdown?.totalCirculation,
            subtotal: campaignData.pricingBreakdown?.subtotal,
            final_total: quotePayload.final_total,
            monthly_price: quotePayload.monthly_price,
  ...
  const handleContactInfoBook = async (contactData: any) => {
    ...
    try {
      ...
      const bookingPayload = {
        ...
        pricing_breakdown: { ...(campaignData.pricingBreakdown ? JSON.parse(JSON.stringify(campaignData.pricingBreakdown)) : {}), discount: bookingDiscountBlock },
          selections: {
            ...
            selectedStartingIssue: campaignData.selectedStartingIssue || null,
      ...
      // Discount-code redemption is recorded server-side after payment is confirmed
      // (see stripe-webhook and create-gocardless-payment edge functions). Doing it
      // here would consume the code even on abandoned checkouts.
