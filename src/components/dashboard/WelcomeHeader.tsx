...
export default function WelcomeHeader({
  ...
  const hasPaidBookings = PREVIEW_AS_PAID ? bookings.length > 0 : bookings.some(b => b.payment_status === 'paid' || b.payment_status === 'subscription_active' || b.payment_status === 'mandate_active');

  // Check if there are unpaid bookings
  const hasUnpaidBookings = PREVIEW_AS_PAID ? false : bookings.some(b => !b.payment_status || b.payment_status === 'pending');

  // Calculate total circulation from paid bookings
  const paidBookingsReach = bookings.filter(b => PREVIEW_AS_PAID || b.payment_status === 'paid' || b.payment_status === 'subscription_active' || b.payment_status === 'mandate_active').reduce((sum, booking) => sum + (booking.total_circulation || 0), 0);

  // Calculate total circulation from unpaid bookings
  const unpaidBookingsReach = bookings.filter(b => !PREVIEW_AS_PAID && (!b.payment_status || b.payment_status === 'pending')).reduce((sum, booking) => sum + (booking.total_circulation || 0), 0);

  // Calculate total investment from unpaid bookings
  const unpaidBookingsInvestment = bookings.filter(b => !PREVIEW_AS_PAID && (!b.payment_status || b.payment_status === 'pending')).reduce((sum, booking) => sum + (booking.final_total || 0), 0);
  ...
  return <div className="mb-8">
      <div className="text-center mb-6">
        ...
        <p className="text-xl text-muted-foreground mb-4">
          {hasPaidBookings && paidBookingsReach > 0 ? `Your advert is in the process of being seen by ${paidBookingsReach.toLocaleString()} number of homes` : unpaidBookingsReach > 0 ? `You're one step away from reaching up to 142,000 Discover readers' homes!` : totalReach > 0 ? `You're one step away from reaching up to 142,000 Discover readers' homes!` : "Start creating your advertising campaign today!"}
        </p>
      </div>
