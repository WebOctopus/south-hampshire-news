...
interface AdvertisingCalculatorProps {
  children: React.ReactNode;
}

const AdvertisingCalculator = ({ children }: AdvertisingCalculatorProps) => {
  ...
  const effectiveSelectedAreas = useMemo(() => 
    pricingModel === 'bogof' ? bogofPaidAreas : formData.selectedAreas,
    [pricingModel, bogofPaidAreas, formData.selectedAreas]
  );
  ...
  const pricingBreakdown = useMemo(() => 
    ...
    [effectiveSelectedAreas, formData.adSize, formData.duration, pricingModel, areas, adSizes, durations, subscriptionDurations, volumeDiscounts]
  );
  ...
  const handleSubmit = () => {
    ...
    if (pricingModel !== 'bogof' && !formData.duration) {
      toast({
        variant: "destructive",
          ...
          <div className="space-y-6">
            ...
            <Card>
              <CardContent className="p-6">
                ...
                {isLoading ? (
                  ...
                  <Select value={formData.adSize} onValueChange={(value) => setFormData(prev => ({ ...prev, adSize: value }))}>
                    <SelectTrigger>
                    ...
                    <SelectContent>
                      {adSizes
                        .filter(size => {
                          ...
                          return size.available_for.includes(pricingModel);
                        })
                        .map((size) => (
                    ...
                    {pricingModel === 'bogof' && (
                      <div className="mt-4 p-4 bg-green-50 rounded-lg border border-green-200">
                        <p className="text-sm font-medium text-green-800">
                          BOGOF Benefit: You're getting {bogofFreeAreas.length} additional areas FREE for 6 months!
                        </p>
                      </div>
                    )}
                  </div>
            ...
            <div className="flex justify-center">
              <Button 
                ...
                size="lg" 
                className="bg-community-green hover:bg-green-600 text-white px-12 py-3 font-bold"
                disabled={!formData.fullName || !formData.emailAddress || !formData.phoneNumber || 
                         effectiveSelectedAreas.length === 0 || !formData.adSize || 
                         (pricingModel !== 'bogof' && !formData.duration)}
              >
                Get Your Quote
              </Button>
            </div>
